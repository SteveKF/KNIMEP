...
public class SkylineVisualizerViewPanel extends JPanel {
	...
	public enum DataType {
		DOMINATEDPOINT, UNDOMINATEDPOINT
	};

	/**
	 * Constructor which chooses chart name, subtitle, etc. according to the option
	 * @param dominatedPoints- list of dominated points (can be also dominated + undominated points)
	 * @param undominatedPoints - list of undomianted points
	 * @param dimensions - string array with dimensions for labeling the axes
	 * @param option - option to choose which chart name, subtitle, etc. to use
	 * @param chartName - custom chart name if "Custom" is the option
	 * @param subtitleText - custom subtitle text if "Custom" is the option
	 * @param dominatedName - custom name for domianted points if "Custom" is the option
	 * @param undominatedName - custom name for undominated points if "Custom" is the option
	 */
	public SkylineVisualizerViewPanel(List<SkylineStructure> dominatedPoints, List<SkylineStructure> undominatedPoints,
			String[] dimensions, String option, String chartName, String subtitleText,
		...
		}else if(option.equals(SkylineVisualizerNodeDialog.options[1])){
			this.chartname = "Representative Skyline Graph";
			this.subtitleText = "This chart shows the skyline points and the representative skyline points in every dimension combination which was selected.";
			this.dominatedName = "Skyline";
			this.undominatedName = "Representative Skyline Points";
		}else{
			this.chartname = chartName;
			this.subtitleText = subtitleText;
			this.dominatedName = dominatedName;
	...
	private JFreeChart createChart(int[] indexes) {
		...
		XYDataset dataset = createDataset(dominatedName, DataType.DOMINATEDPOINT, indexes);

		JFreeChart chart = ChartFactory.createXYLineChart(chartname, dimensions[indexes[0]],
				dimensions[indexes[1]], dataset, PlotOrientation.VERTICAL, true, true, false);

		if (indexes == INDEXES_XY) {

			chart = ChartFactory.createXYLineChart(chartname, dimensions[indexes[0]],
					dimensions[indexes[1]], dataset, PlotOrientation.VERTICAL, true, true, false);
		...
		XYDataset dataset2 = createDataset( undominatedName, DataType.UNDOMINATEDPOINT, indexes);
	 ...
	 * @param type - data type (DOMINATEDPOINT or UNDOMINATEDPOINT)
	 * @param indexes - column indexes which will be considered
	 * @return
	 */
	 ...
	 * @param list - the data which will be added to the series
	 * @param indexes - the column indexes which are being considered
	 */
	private void addSeriesPoint(XYSeries series, List<SkylineStructure> list, int[] indexes) {
